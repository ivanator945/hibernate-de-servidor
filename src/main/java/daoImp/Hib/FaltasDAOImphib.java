...
public class FaltasDAOImphib implements IFaltasDAO {
	...
	public ArrayList<FaltaDTO> obtenerFaltasPorFiltros(String nombreAlumno, String asignatura, String fecha,
		...
		String jpql = "SELECT new dto.FaltaDTO(" + "f.idFalta, " + "a.id, a.nombre, " + "asig.id, asig.nombre, "
				+ "f.fecha, f.justificada) " + "FROM FaltasEntity f " + "JOIN f.alumno a " + "JOIN f.asignatura asig "
				+ "WHERE a.nombre LIKE :nombreAlumno " + "AND asig.nombre LIKE :asignatura " + "AND f.fecha >= :fecha "
	...
	public ArrayList<FaltaDTO> obtenerFaltasPorFiltrosSinFecha(String nombreAlumno, String asignatura,
		...
		String jpql = "SELECT new dto.FaltaDTO(" + "f.idFalta, " + "a.id, a.nombre, " + "asig.id, asig.nombre, "
				+ "f.fecha, f.justificada) " + "FROM FaltasEntity f " + "JOIN f.alumno a " + "JOIN f.asignatura asig "
				+ "WHERE a.nombre LIKE :nombreAlumno " + "AND asig.nombre LIKE :asignatura "
				+ "AND f.justificada = :justificada";
